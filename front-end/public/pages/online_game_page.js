...
export default class OnlineGamePage extends HTMLElement {
  ...
  connectedCallback() {
    this.websocket = new WebSocket(`${ips.socketUrl}/ws/lobby/lobby/`);
    this.websocket.onmessage = function (e) {
      ...
      if (message.game_data) {
        ...
        if (message.game_data.player1) {
          ...
          state[0].player1.paddle_theme =
            message.game_data.player1.paddle_theme;
        }
        if (message.game_data.player2) {
          state[0].player2.name = message.game_data.player2.name;
          state[0].player2.paddle_theme =
            message.game_data.player2.paddle_theme;
        }
    ...
    this.innerHTML = /*html*/ `
		<div class="game_page">
			...
			<div class="game_theme">
				<h1>Table Theme</h1>
				<div class="themes">
					<button id="classic" class="theme" ${
            this.role === "player2" ? "disabled" : ""
          }></button>
					<button id="mod" class="theme selected_theme" ${
            this.role === "player2" ? "disabled" : ""
          }></button>
					<button id="sky" class="theme" ${
            this.role === "player2" ? "disabled" : ""
          }></button>
				</div>
			</div>
            ...
            JSON.stringify({
              game_data: {
                player2: {
                  ...
                  paddle_theme: localStorage.getItem("state")
                    ? JSON.parse(localStorage.getItem("state"))[0].player2
                        .paddle_theme
                    : "classic",
                },
                ready: true,
            ...
            JSON.stringify({
              game_data: {
                player1: {
                  ...
                  paddle_theme: localStorage.getItem("state")
                    ? JSON.parse(localStorage.getItem("state"))[0].player1
                        .paddle_theme
                    : "classic",
                },
                ready: true,
  ...
  updateReadyButton(buttonId, isReady) {
    ...
    if (isReady) button.classList.add("active");
